...
Message.prototype.add = async function (message) {
  try {
    var msid = message.time + ":" + crypto.randomBytes(7).toString("hex")
 ...
 *   @param {string} where.msid
 **/
Message.prototype.listFromRoom = async function (where) {
  var messages

  try {
    messages = await this.dynamoDB
      .query({
        TableName: this.tableName,
        ...
        ExclusiveStartKey: where.msid ? where : undefined,
